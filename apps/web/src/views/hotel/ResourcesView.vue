...
const items = ref<PartnerResource[]>([]); const loading = ref(false)
...
async function toggle(row: PartnerResource) { try { await hotelApi.toggleResourcePackage(row.id, !row.package_enabled); ElMessage.success('组包许可已更新'); await load() } catch (e) { ElMessage.error(errorMessage(e)) } }
onMounted(load)
</script>
