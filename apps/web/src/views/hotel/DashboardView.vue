...
const loading = ref(true); const error = ref(''); const dashboard = ref<Dashboard | null>(null); const products = ref<TravelProduct[]>([]); const resources = ref<PartnerResource[]>([]); const chartEl = ref<HTMLElement | null>(null); let chart: echarts.ECharts | undefined; let socket: WebSocket | undefined
...
async function load() { loading.value = true; error.value = ''; try { const [summary, productResponse, resourceResponse] = await Promise.all([hotelApi.dashboard(), hotelApi.products(), hotelApi.resources()]); dashboard.value = summary.data; products.value = productResponse.data.items; resources.value = resourceResponse.data.filter(item => item.package_enabled && item.status === 'AVAILABLE'); await nextTick(); renderChart() } catch (e) { error.value = errorMessage(e) } finally { loading.value = false } }
function openResources() { window.location.href = '/hotel/resources' }
...
<template>
  ...
  <template v-else-if="dashboard">
    <div class="metric-grid"><div class="metric-link" @click="$router.push('/hotel/rooms')"><MetricCard label="临期客房" :value="dashboard.available_room_units" hint="明日待售房量 · 点击查看" accent="#0f766e" /></div><div class="metric-link" @click="openResources"><MetricCard label="可组包资源" :value="dashboard.package_enabled_resource_count" :hint="`合作资源 ${dashboard.partner_resource_count} 项 · 点击查看`" accent="#c9963e" /></div><div class="metric-link" @click="$router.push('/hotel/products')"><MetricCard label="在售产品" :value="dashboard.on_sale_product_count" :hint="`共 ${dashboard.product_count} 个产品 · 点击查看`" accent="#498c70" /></div><div class="metric-link" @click="$router.push('/hotel/intents')"><MetricCard label="在售毛利" :value="`¥${dashboard.gross_profit_on_sale}`" hint="按当前产品库存估算 · 点击查看意向" accent="#7c6ab0" /></div></div>
    <div class="section-title"><h2>可组包资源</h2><span>点击卡片查看资源详情与组包许可</span></div>
    <div v-if="resources.length" class="resource-snapshot-grid"><button v-for="resource in resources" :key="resource.id" class="resource-snapshot" @click="openResources"><div class="product-card__top"><strong>{{ resource.resource_name }}</strong><span :class="resource.remaining_capacity <= 5 ? 'warning-text' : 'muted'">余 {{ resource.remaining_capacity }}</span></div><p class="muted">{{ resource.available_date }} · {{ resource.start_time?.slice(0,5) || '--' }} - {{ resource.end_time?.slice(0,5) || '--' }}</p><div class="product-card__resources"><span>{{ resource.merchant_name }}</span><span>¥{{ resource.settlement_price }}</span></div></button></div><div v-else class="panel empty-state">暂无可组包资源，点击“生成主题产品”前请先在资源池打开许可。</div>
...
<style scoped>
.metric-link{cursor:pointer;transition:transform .2s}.metric-link:hover{transform:translateY(-3px)}.resource-snapshot-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.resource-snapshot{border:1px solid var(--line);border-radius:12px;background:#fff;padding:16px;text-align:left;color:var(--ink);cursor:pointer;box-shadow:var(--shadow);transition:.2s}.resource-snapshot:hover{border-color:#7bb8a8;box-shadow:0 15px 35px rgba(30,72,64,.12)}.resource-snapshot p{margin:11px 0}@media(max-width:900px){.resource-snapshot-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:600px){.resource-snapshot-grid{grid-template-columns:1fr}}
</style>
