...
- 新增回归：组包许可显式开关；临期客房新增/编辑日期；多候选方案不产生错误的交叉冲突；商户新增/编辑资源名称、日期和起止场次；多方案生成与营销素材；产品编辑、重算、删除；自然语言需求解析与推荐确认卡。
...
当前自动化测试结果：`20 passed, 1 warning`。唯一 warning 来自 Starlette 测试客户端依赖的上游弃用提示，不影响业务测试。
