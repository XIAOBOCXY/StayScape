...
Docker Compose 的 `demo` profile 不启动 Gateway，使用 Mock Agent 完成可重复演示；`live` profile 构建固定版本的官方 `ghcr.io/openclaw/openclaw:2026.5.29`，Gateway 只在 Docker 内网监听 18789。Nginx 只代理 Web/API/WebSocket，不代理 `/v1/responses`。
